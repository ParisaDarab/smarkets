import { useState, type FormEvent } from 'react';
import { useNavigate } from 'react-router-dom';
import { CustomInput } from '@/components/input';
import { Button } from '@/shadcn/components/ui/button';
import { login, buttons, inputs } from '@/lib/i18n/en';
import { useAuth } from '@/hooks/useAuth';
import { useLogin } from '@/hooks/auth/useLogin';
import { ApiError } from '@/api/apiClient';
import { validateEmail, validatePassword } from '@/lib/validation/validation';
import { PasswordRequirements } from '@/components/PasswordRequirements';
export const Login = () => {
  const navigate = useNavigate();
  const { login: setSession } = useAuth();
  const loginMutation = useLogin();
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [submitted, setSubmitted] = useState(false);
  const [formError, setFormError] = useState<string | null>(null);
  const submitHandler = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setSubmitted(true);
    setFormError(null);

    if (validateEmail(username) || validatePassword(password)) {
      return;
    }

    loginMutation.mutate(
      { username, password },
      {
        onSuccess: (response) => {
          if (response.factor && response.factor !== 'complete') {
            setFormError(login.errors.mfaNotSupported);
            return;
          }

          if (!response.token) {
            setFormError(login.errors.missingToken);
            return;
          }

          setSession(response.token);
          navigate('/', { replace: true });
        },
        onError: (error) => {
          if (error instanceof ApiError && error.status === 401) {
            setFormError(login.errors.invalidCredentials);
            return;
          }
          setFormError(
            error instanceof Error ? error.message : login.errors.generic
          );
        },
      }
    );
  };

  return (
    <div className="h-screen overflow-hidden">
      <div className="p-7 shadow-lg bg-white">
        <div className="w-36">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 144 32"
            className="SmarketsLogo_logo__UCtGT"
          >
            <path
              fill="var(--logo-primary)"
              fillRule="evenodd"
              d="M125.942 27.1h-3.604V13.174h-3.006v-3.098h3.006V4.904h3.604v5.173h3.542v3.098h-3.542zm11.407.192c-1.411 0-2.898-.429-4.083-1.266-1.442-1.019-2.27-2.697-2.27-4.46h3.331l.004.048c.034.658.338 1.284.867 1.68q.158.117.334.205l.01.006c.725.434 1.558.613 2.547.547 1.411-.1 2.344-.89 2.378-2.01.027-.95-.674-1.6-2.084-1.931l-2.29-.536c-3.416-.922-4.878-2.705-4.606-5.613v-.001c.254-2.47 2.877-4.405 5.969-4.405 2.412 0 4.263.907 5.232 2.496.453.741.667 1.604.662 2.473l-.001.092h-3.477a1.69 1.69 0 0 0-.73-1.45c-.422-.284-.979-.443-1.614-.443-1.149 0-2.11.565-2.336 1.373-.137.511-.129.86.026 1.202.247.52.82.888 1.858 1.19l2.462.606c2.035.502 4.462 1.782 4.462 5.025 0 1.534-.695 2.85-2.008 3.804-1.214.882-2.863 1.368-4.643 1.368m-90.987-.184h-3.604v-9.6c0-2.663-1.367-4.253-3.655-4.253-1.165-.02-2.177.368-2.928 1.108-.86.847-1.334 2.116-1.334 3.573v9.172h-3.604v-9.636c0-2.601-1.318-4.217-3.44-4.217-2.164 0-4.476 1.23-4.476 4.681v9.172h-3.605V10.086h3.605v2.038c.44-.62 1.078-1.154 1.878-1.568.793-.41 1.746-.692 2.55-.755.658-.037 1.285 0 1.87.11 2.037.31 3.499 1.33 4.345 3.033 1.528-2.647 3.97-3.215 5.784-3.215 2.123 0 3.779.674 4.922 2.002 1.123 1.305 1.692 3.177 1.692 5.563zm11.694-13.854c-3.05 0-5.264 2.225-5.264 5.289 0 3.076 2.263 5.396 5.264 5.396 2.587 0 5.371-1.688 5.371-5.396 0-3.435-2.767-5.289-5.37-5.289m-.286 14.211c-2.326 0-4.486-.86-6.08-2.424-1.66-1.628-2.574-3.923-2.574-6.462 0-2.587.89-4.82 2.575-6.458 1.587-1.542 3.746-2.392 6.079-2.392 2.366 0 4.53.95 5.693 2.461v-2.104h3.57v17.022h-3.57v-2.172c-1.18 1.552-3.346 2.529-5.693 2.529m43.249-.356h-4.773l-7.584-8.083v8.081h-3.796V0h3.796v15.678l5.29-5.61h4.793l-6.882 7.283zm-26.668-.001h-3.785V10.086h3.786v2.015c1.204-1.337 2.764-2.015 4.642-2.015 1.284.022 2.441.39 3.442 1.095.122.086.412.274.564.371a.103.103 0 0 1 .024.151l-2.69 3.33a.17.17 0 0 1-.215.042l-.489-.278c-.633-.363-1.244-.604-1.886-.604-.887 0-1.657.301-2.288.895l-.008.007c-.706.58-1.075 1.541-1.096 2.856z"
              clipRule="evenodd"
            ></path>
            <mask
              id="smarkets-logo_svg__a"
              width="118"
              height="32"
              x="0"
              y="0"
              maskUnits="userSpaceOnUse"
              className="mask-type:luminance"
            >
              <path
                fill="#fff"
                fillRule="evenodd"
                d="M117.9 31.996H0V0h117.9z"
                clipRule="evenodd"
              ></path>
            </mask>
            <g
              fillRule="evenodd"
              clipRule="evenodd"
              mask="url(#smarkets-logo_svg__a)"
            >
              <path
                fill="var(--logo-primary)"
                d="M104.337 17.12s.022-.148.058-.304c.255-1.094.814-2 1.6-2.654.842-.7 1.943-1.11 3.211-1.153q.109-.004.219-.004c2.481 0 4.288 1.448 4.816 3.814l.067.301zm11.147-5c-1.521-1.542-3.674-2.39-6.059-2.39-1.008 0-1.979.15-2.884.437a8.4 8.4 0 0 0-2.846 1.543q-.234.195-.452.407c-1.664 1.618-2.544 3.84-2.544 6.427 0 5.169 3.669 8.92 8.726 8.92 2.623 0 4.971-.968 6.555-2.65l-2.278-2.43c-.96 1.137-2.489 1.806-4.169 1.806-2.595 0-4.616-1.545-5.148-3.933a.207.207 0 0 1 .201-.254H117.9v-1.46c0-2.6-.836-4.822-2.416-6.423"
              ></path>
              <path
                fill="var(--logo-accent)"
                d="m15.72 0-.262.658-1.81 4.475-.072-.054a8.6 8.6 0 0 0-2.23-1.152L12.942 0zM4.216 28.446l-1.445 3.557H-.007l.262-.658 1.724-4.266a9.3 9.3 0 0 0 2.237 1.367"
              ></path>
            </g>
            <path
              fill="var(--logo-primary)"
              fillRule="evenodd"
              d="M.448 11.207c0-4.053 3.774-6.475 7.422-6.475 4.126 0 7.086 2.472 7.367 6.149.026.273.026.814.026.814h-3.44s-.006-.41-.062-.732l-.031-.19a3 3 0 0 0-.132-.477 3.1 3.1 0 0 0-1.222-1.54 3 3 0 0 0-.24-.146 1 1 0 0 0-.088-.048 3 3 0 0 0-.228-.11 3 3 0 0 0-.233-.096l-.005-.002a4.7 4.7 0 0 0-1.177-.266 6 6 0 0 0-.567-.028c-1.93 0-3.927 1.165-3.927 3.114 0 .478 0 1.838 2.186 2.627L4.86 16.865c-2.93-1.05-4.412-2.95-4.412-5.658m15.454 10.741c-.435 3.585-3.508 5.994-7.646 5.994-2.073 0-4.028-.678-5.506-1.91C1.33 24.848.416 23.197.285 21.581c-.033-.406 0-.828 0-.828h3.378s.012.27.066.553c.043.23.12.499.166.618l.03.078a3 3 0 0 0 .14.315l.04.078q.02.04.044.078a4 4 0 0 0 .285.43 4.2 4.2 0 0 0 .995.937c.42.287.909.517 1.455.662a5.3 5.3 0 0 0 1.372.177c1.826 0 3.9-.801 4.25-3.045.172-2.167-1.619-3.133-3.024-3.555l1.239-3.066c1.697.55 3 1.338 3.888 2.349 1.088 1.24 1.523 2.784 1.293 4.585"
              clipRule="evenodd"
            ></path>
            <path
              fill="var(--logo-accent)"
              fillRule="evenodd"
              d="m10.67 12.585-.281.69L6.576 22.7a3 3 0 0 1-.196-.124l-.044-.03-.075-.056c-1.38-.976-1.175-2.776-1.122-3.119a.5.5 0 0 1 .03-.106l.38-.938 3.606-8.91.256.118c1.366.707 1.258 3.05 1.258 3.05"
              clipRule="evenodd"
            ></path>
          </svg>
        </div>
      </div>

      <section className="py-3 flex flex-col justify-center gap-10 items-center w-full md:flex-row md:items-stretch">
        <div className=" w-[80%] md:w-1/3 p-5">
          <form
            onSubmit={submitHandler}
            noValidate
            className="bg-white rounded-lg flex flex-col gap-7 p-7"
          >
            <strong className="font-medium text-2xl">{login.title}</strong>
            <CustomInput
              className="h-14"
              id="emailInput"
              ariaLabel={login.emailLabel}
              value={username}
              onchange={setUsername}
              validate={validateEmail}
              forceValidate={submitted}
              disabled={loginMutation.isPending}
              required
            />
            <CustomInput
              className="h-14"
              id="passwordInput"
              type="password"
              placeholder={inputs.passwordPlaceholder}
              ariaLabel={login.passwordLabel}
              value={password}
              onchange={setPassword}
              validate={validatePassword}
              forceValidate={submitted}
              disabled={loginMutation.isPending}
              required
            />
            <PasswordRequirements value={password} />
            {formError && <p className="text-red-500 text-sm">{formError}</p>}
            <Button
              type="submit"
              className={`bg-smarketsGray h-12 ${!validateEmail(username) && !validatePassword(password) && 'bg-smarketsGreen'}`}
              disabled={loginMutation.isPending}
            >
              {loginMutation.isPending ? login.submitting : buttons.continue}
            </Button>
            <p className="text-gray-600  text-[12.5px]">
              By clicking "Continue", I accept Smarket's Terms & Privacy Policy
              and confirm that I am 18 years or older. Smarkets (Malta) Limited
              is licensed and regulated in Great Britain by the Gambling
              Commission under account number 39173. Licensed and regulated
              outside of the UK by Malta Gaming Authority (MGA/CRP/162/2008
              issued May 12th, 2009).
            </p>
          </form>
        </div>
        <div className="hidden md:block w-1/2">
          <img
            src="/assets/images/american-football-light.webp"
            alt="Football"
          />
        </div>
      </section>
    </div>
  );
};
